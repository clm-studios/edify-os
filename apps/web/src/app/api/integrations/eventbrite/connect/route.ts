...
/**
 * GET /api/integrations/eventbrite/connect
 *
 * Redirects the user to Eventbrite's OAuth2 authorization screen.
 * Mirrors the shape of /api/integrations/mailchimp/connect.
 */
export async function GET() {
  ...
  if (!clientId || !clientSecret) {
    return NextResponse.json(
      { error: 'Eventbrite OAuth credentials not configured on server' },
      { status: 500 }
    );
  }

  // Pin redirect_uri — must EXACTLY match what is registered in the Eventbrite app
  ...
  cookieStore.set(EVENTBRITE_STATE_COOKIE, state, {
    httpOnly: true,
